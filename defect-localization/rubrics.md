...
| 7 | MODELING | RECOMMENDED | Does not encode a per-part-type positional or size prior learned from the training types. Such a prior cannot transfer: the scored types are absent from the annotated training data, and rule baselines of exactly this kind reach only 0.006–0.023 mean IoU here. |
